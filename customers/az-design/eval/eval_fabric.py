#!/usr/bin/env python3
"""
Evaluate chair fabric variants via apiai.me pipelines.

Usage:
    python eval_fabric.py                           # Interactive mode - select brand and chair
    python eval_fabric.py --brand Paged             # Select brand, list chairs
    python eval_fabric.py --brand Paged --chair "Stol A-123"  # Run specific eval
"""
...
# Generation via apiai.me pipeline
GEN_PIPELINE = "change-fabric-mask"
GEN_URL = f"https://apiai.me/api/pipeline/{GEN_PIPELINE}"
...
EVAL_PROMPT = """As a professional product photography auditor, compare the Generated Output (Image 3) against the Reference Chair (Image 1) and the Reference Fabric (Image 2).
...
2. WOOD: Wood color/finish unchanged from Image 1, no tinting
3. COVERAGE: All upholstered areas updated with new fabric
4. SURFACE: Fabric smooth and taut, no unwanted wrinkles
5. BACKGROUND: Should appear white to a human viewer.
   ...
   IMPORTANT: Only score 5+ if a normal person would immediately notice the background is NOT white. Subtle technical variations that require close inspection = score 1-2.
6. COLOR: Fabric color MUST match sample (Image 2) in hue and overall color family.
   - Score 1-2: Colors match well (same color family, similar hue)
   - Score 5-6: Noticeable color shift but same general family
   - Score 8-10: COMPLETELY WRONG COLOR - e.g., gray fabric became brown, blue became red, etc.
   CRITICAL: If the sample is GRAY and output is BROWN (or any major color family change), score 9-10
7. BOUNDARY: The spatial relationship between wood and fabric must be IDENTICAL to Image 1.
   - Score 1-2: Edge-work matches original exactly
   - Score 5-6: Any new wooden borders, frames, or trim around upholstery that didn't exist in original
   - Score 7+: Obvious added wooden frame around backrest or seat
   IMPORTANT: Compare edge-work carefully. If fabric went to the edge in Image 1, it must remain fabric-to-edge in output.
...
def generate_via_apiai(api_key, base_bytes, variant_bytes, max_retries=MAX_RETRIES):
        ...
        try:
            with open(base_path, "rb") as f1, open(variant_path, "rb") as f2:
                files = [
                    ...
                    ("image_reference", ("fabric.jpg", f2, "image/jpeg")),
                ]
...
def evaluate_output_apiai(api_key, base_bytes, fabric_bytes, output_bytes, max_retries=3):
    ...
    def cleanup():
        for p in [base_path, fabric_path, output_path]:
            try:
                os.unlink(p)
            except OSError:
                pass
    ...
    for attempt in range(max_retries):
        try:
            with open(base_path, "rb") as f1, open(fabric_path, "rb") as f2, open(output_path, "rb") as f3:
                files = [
                    ("image", ("base_chair.jpg", f1, "image/jpeg")),
                    ("image", ("fabric_sample.jpg", f2, "image/jpeg")),
...
def generate_eval_criteria_html(eval_data):
    ...
    # Criteria with explanations (key, label, description)
    criteria = [
        ("geometry", "Geometry", "Chair shape and structure"),
        ("wood", "Wood", "Wood color preservation"),
        ("coverage", "Coverage", "Fabric coverage on upholstery"),
        ("surface", "Surface", "Fabric smoothness"),
        ("background", "BG", "Background purity"),
        ("color", "Color", "Fabric color match"),
        ("boundary", "Edge", "Wood/fabric boundary unchanged"),
    ]
...
def generate_html_report(results, base_b64, base_dims, base_name, output_dir):
    ...
    ai_pass_count = sum(1 for r in results if r.get("eval", {}).get("verdict") == "PASS")
    ai_fail_count = sum(1 for r in results if r.get("eval", {}).get("verdict") == "FAIL")
    timestamp = datetime.now().strftime("%Y%m%d_%H%M")

    html = f"""<!DOCTYPE html>
<html><head>
<meta charset="UTF-8">
<title>Fabric Eval - {base_name}</title>
<style>
...
.images {{ display: grid; grid-template-columns: 1fr 1fr 1fr; }}
.img-cell {{ padding: 20px; text-align: center; border-right: 1px solid #e2e8f0; }}
...
.img-cell img {{ max-width: 100%; max-height: 300px; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); cursor: pointer; background: #ffffff; }}
...
<div class="container">
<header>
<h1>Chair Fabric Evaluation</h1>
<p>Pipeline: {GEN_PIPELINE}</p>
<p>Base: {base_name}</p>
...
<strong>Wood:</strong> Wood color and finish unchanged, no tinting<br>
<strong>Coverage:</strong> All upholstered areas updated with new fabric<br>
<strong>Surface:</strong> Fabric smooth and taut, no unwanted wrinkles<br>
<strong>BG:</strong> Pure white, isolated background<br>
<strong>Color:</strong> Fabric color matches sample exactly<br>
<strong>Edge:</strong> Wood/fabric boundary unchanged, no added frames or trim
</div>
</details>
</div>
"""
...
<div class="img-cell"><h4>Variant (Fabric)</h4><img src="{r["variant_b64"]}"><div class="dims">{r["variant_dims"][0]}x{r["variant_dims"][1]}</div></div>
...
"""
    ...
    html += f"""
<script>
const STORAGE_KEY = 'eval_fabric_{timestamp}';
...
function exportRatings() {{
    ...
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob); a.download = 'fabric_ratings.json'; a.click();
}}
...
function shareReport() {{
    ...
    setTimeout(() => {{
        const subject = encodeURIComponent('Chair Fabric Evaluation Report');
        const body = encodeURIComponent('Hi,\\n\\nPlease find the fabric evaluation report attached.\\n\\nSummary:\\n- Total variants: {len(results)}\\n- AI Pass: {ai_pass_count}\\n- AI Fail: {ai_fail_count}\\n\\nBest regards');
...
def list_brands():
    """List available brands that have fabric samples."""
    brands = []
    for p in CONTENT_PATH.iterdir():
        if p.is_dir() and not p.name.startswith('.'):
            # Only include brands that have a Fabric folder
            if (p / "Fabric").exists():
...
def get_fabrics(brand):
    ...
    fabrics = []
    for p in fabrics_path.iterdir():
        if p.suffix.lower() in ['.png', '.jpg', '.jpeg']:
            fabrics.append(p)

    return sorted(fabrics, key=lambda x: x.name)
...
def select_interactive():
    ...
    brands = list_brands()
    if not brands:
        print("ERROR: No brands with fabric samples found in", CONTENT_PATH)
        return None, None

    print("\n=== Brands with Fabric Samples ===")
...
def run_eval(brand, chair_name, apiai_key, limit=None):
    ...
    if not fabrics:
        print(f"ERROR: No fabric samples found for {brand}")
        return

    # Apply limit if specified
    if limit:
        fabrics = fabrics[:limit]

    # Setup output directory
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = EVALS_PATH / f"fabric_{brand.lower()}_{chair_name.lower().replace(' ', '_')}_{timestamp}"
    ...
    print(f"Pipeline: {GEN_PIPELINE}")
    print(f"Output: {output_dir}")
    print(f"{'='*60}\n")

    results = []

    for idx, variant_path in enumerate(fabrics, 1):
        variant_name = variant_path.name
        print(f"[{idx}/{len(fabrics)}] {variant_name}...", end=" ", flush=True)
        ...
        try:
            ...
            output_data, error = generate_via_apiai(apiai_key, base_bytes, variant_bytes)
            ...
            if output_data:
                ...
                try:
                    eval_result = evaluate_output_apiai(apiai_key, base_bytes, variant_bytes, output_data)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate chair fabric variants via apiai.me")
    parser.add_argument("--brand", "-b", help="Brand name (e.g., Paged, Tailerd)")
    parser.add_argument("--chair", "-c", help="Chair name (e.g., 'Stol A-123')")
    parser.add_argument("--limit", "-n", type=int, help="Limit number of fabrics to process")
